...
def main(gt_dir: Path, pred_dir: Path, out_dir: Path, is_skipped: bool = False):
    loader = ImageLoaderService(gt_dir=gt_dir, pred_dir=pred_dir)

    if len(loader.gt_index) == 0 or len(loader.pred_index) == 0:
        raise ValueError(
            f"No valid images found. GT={len(loader.gt_index)}, Pred={len(loader.pred_index)}. "
        )

    logger.info(f"Found {len(loader.gt_index)} GT images")
    logger.info(f"Found {len(loader.pred_index)} Pred images")
    ...
    results = evaluate(loader, metrics)
...
def evaluate(loader, metrics):
    results = {}
    pairs = list(loader.iter_pairs())
    ...
    for metric in metrics:
        ...
        if kind == "pairwise":
            ...
            gt_dir = str(loader.gt_dir)
            pred_dir = str(loader.pred_dir)
            res = metric.compute(gt_dir, pred_dir)
            results[name] = res
            logger.info(f"{name.upper()}: " + ", ".join(f"{k}={v:.4f}" for k, v in res.items()))
    return results
...
if __name__ == "__main__":
    ...
    main(GT_DIR, PRED_DIR, OUT_DIR, True)