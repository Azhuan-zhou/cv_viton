...
class VitonHDDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if cloth_trim.shape[0]==1:
            cloth_trim = cloth_trim[0]
...
def show_sample(item, idx=0, save_path="preview.png"):
    to_pil = transforms.ToPILImage()
    imgs = [
        ("image", denorm(item["image"])),
        ("cloth_pure", denorm(item["cloth_pure"])),
        ("im_mask", denorm(item["image"] * (item["inpaint_mask"] == 1))),
        #("im_mask", denorm(item["im_mask"])),
        ("inpaint_mask", item["inpaint_mask"]),  # 0/1
        ("pose_img", denorm(item["pose_img"])),
    ]
    cols = len(imgs)
    plt.figure(figsize=(3*cols, 3))
    for i, (name, tensor) in enumerate(imgs, 1):
        plt.subplot(1, cols, i)
        if tensor.shape[0] == 1:
            plt.imshow(tensor[0].cpu().numpy(), cmap="gray")
        else:
            plt.imshow(to_pil(tensor.cpu()))
        plt.title(name)
        plt.axis("off")
    plt.suptitle(f"{item['c_name']}")
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    ds = VitonHDDataset(dataroot_path="viton-hd", phase="train")
    ...
    for k, v in item.items():
        if torch.is_tensor(v):
            print(f"{k:12s} shape={tuple(v.shape)}  range=({v.min().item():.3f},{v.max().item():.3f})")
        else:
            print(f"{k:12s} type={type(v)}  value={v if isinstance(v,str) else '...'}")


    show_sample(item)


    loader = DataLoader(ds, batch_size=4, shuffle=False, collate_fn=simple_collate, num_workers=0)
    batch = next(iter(loader))
    print("\nBatch check:")
    for k, v in batch.items():
        if torch.is_tensor(v):
            print(f"{k:12s} batch_shape={tuple(v.shape)}")

    b0 = {k: (v[0] if torch.is_tensor(v) else v[0]) for k, v in batch.items()}
    show_sample(b0)